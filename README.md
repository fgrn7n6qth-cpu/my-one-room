...
- Three.js
- @react-three/fiber
- CSS Modules 없이 순수 CSS 기반 UI 스타일링
...
- 하나의 배치 데이터를 2D 편집, 3D 시각화, 프로젝트 저장, 추천 기능에서 공유하도록 설계했습니다.
- 3D 가구가 바닥에서 뜨는 문제를 타입별 높이와 지지면 계산으로 개선했습니다.
- 투어 모드 카메라를 별도로 관리해 사용자가 방 안을 직접 둘러보는 경험을 구현했습니다.
- API 키 없이도 추천 키워드를 쇼핑 검색으로 연결하는 MVP 흐름을 구현했습니다.
